...
def get_children(parent_id,x_base,y_base,basepath,allowedlevels,level,models,kinfos,infosConcat,base_shift):
    ...
    for patch in [upperleft,lowleft,upperright,lowright]:
        x,y=patch
        path= glob.glob(os.path.join(basepath,"*_x_"+str(x)+"_y_"+str(y)+".jpg"))
        ...
        if level in allowedlevels and checkentropy(image):
            ...
            if parent_id is not None:
                concatened= np.concatenate([parent["embedding"],embedding],axis=-1)
                infosConcat.extend(concatened)
            kinfo=encapsulate_patch_info(parent_id=parent_id,x=x,y=y,id=len(kinfos),shift=shift,level=level,embedding=embedding,path=path)
            kinfos.append(kinfo)
            kinfos,infosConcat=get_children(kinfo["id"],x,y,path.split(".")[0],allowedlevels,level+1,models,kinfos,infosConcat,base_shift)
        else:
            kinfos,infosConcat=get_children(parent_id,x,y,path.split(".")[0],allowedlevels,level+1,models,kinfos,infosConcat,base_shift)
...
def search_neighboors(infos):
    ...
    for level in tqdm.tqdm(range(0,4)):
        ...
        for idx in range(df2.shape[0]):
            ...
            y=patch["y"]
            lista=[]
            df3=df2.query("(x=="+str(x+shift)+" & y=="+str(y)+") | (x=="+str(x+shift)+" & y=="+str(y+ shift)+") | (x=="+str(x)+" & y=="+str(y+shift)+")  | (x=="+str(x-shift)+" & y=="+str(y+shift)+")  | (x=="+str(x-shift)+" & y=="+str(y)+")  | (x=="+str(x-shift)+" & y=="+str(y-shift)+")  | (x=="+str(x)+" & y=="+str(y-shift)+")  | (x=="+str(x+shift)+" & y=="+str(y-shift)+")")
    ...
    with torch.no_grad():
        ...
        for path in tqdm.tqdm(low_patches):
            ...
            if level in allowedlevels:
                ...
                kinfo=encapsulate_patch_info(parent_id=None,path=path,x=x,y=y,id=len(infos),shift=shift,level=level,embedding=embedding)
                infos.append(kinfo)
                infos,infos_concat=get_children(kinfo["id"],x,y,path.split(".")[0],args.levels,level+1,models,infos,infos_concat,base_shift)
            else:
                infos,infos_concat=get_children(None,x,y,path.split(".")[0],args.levels,level+1,models,infos,infos_concat,base_shift)
...
def main():
    ...
    models=[]
    weights=[args.pretrained_weights1,args.pretrained_weights2,args.pretrained_weights3]
    for idx in range(3):
        net=copy.deepcopy(model)
        net=net.cuda()
        net.eval()
        utils.load_pretrained_weights(net, weights[idx], args.checkpoint_key, args.arch, args.patch_size)
        models.append(net)
