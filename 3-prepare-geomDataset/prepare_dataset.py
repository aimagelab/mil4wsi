...
class MyOwnDataset(Dataset):
    ...
    def process(self):
        ...
        for idx, bag in enumerate(bags):
            if "test" in bag:
                ispath=os.path.exists(os.path.join(self.processed_dir, "test", 'data_{}.pt'.format(idx)))
            else:
                ispath=os.path.exists(os.path.join(
                        self.processed_dir, "train", 'data_{}.pt'.format(idx)))
            # Get data structure
            if ispath == False:
                try:
                ...
                print(bag, ' at ', idx, ' already transformed. Skipped')
